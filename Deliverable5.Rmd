...
#Best model is using  Pickup_latitude + Dropoff_longitude + Trip_distance + MTA_tax? + Tolls_amount?, ? make sence?
```{r}
...
Boxplot(rstudent(m51),id.n=15)

llout<-which(abs(rstudent(m51))>2,8);length(llout)
dfwork[llout,] 

# Observacions potencialment influents
quantile(hatvalues(m51),seq(0,1,0.1))

mean(hatvalues(m51))
hh<-5*mean(hatvalues(m51))
llhat<-which(hatvalues(m51)>hh);length(llhat)
hh
Boxplot(hatvalues(m51),labels=rownames(dfwork))
