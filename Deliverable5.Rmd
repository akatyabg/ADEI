...
m2<-glm(AnyTip~ Dropoff_longitude + Trip_distance:f.total, family="binomial",data=dfwork)
summary(m2)
vif(m2)
...
maux<-glm(AnyTip~ Dropoff_longitude + f.total + Trip_distance:f.total, family="binomial",data=dfwork)
summary(maux)
vif(maux)
BIC(maux)

m59<-glm(AnyTip~Dropoff_longitude + f.total, family="binomial",data=dfwork)
summary(m59)
vif(m59)
BIC(m51,m59)
...
## Diagnostics
Is there any homoscedascity?
### Residuals
```{r}

xpea<-sum(residuals(m51,'pearson')^2);xpea
xpea<-sum(residuals(m58,'pearson')^2);xpea
xpea<-sum(residuals(m63,'pearson')^2);xpea
xpea<-sum(residuals(m64,'pearson')^2);xpea


#Residuals
residualPlots(m51, layout=c(1, 3)) 
...
Boxplot(cooks.distance(m51)) #connections with influence data
