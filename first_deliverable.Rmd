...
```{r}
names(df)
vars_con<-names(df)[c(6:9, 11,12,15,16,24:26,29)] 
vars_con
...
vars_dis_at <- c(vars_dis, "AnyTip") 
vars_con_catdes <- c(vars_con_and_dis,(names(df)[c(23)]))
#if p.value is under 0.5 then we have to reject the hipothesis of no relation between variable and target
catdes(df[,vars_con_catdes ],num.var = 39)
```
