...
```
```{r,include=FALSE}
setwd("/home2/users/alumnes/1171633/Desktop/ADEI/ADEI") #Change 
...
outlier<-which(df$Passenger_count>6.0);length(outlier)
...
#How to detect missing values? 0.0 is a possible value? no 
...
summary(df$f.latitudedeDrop) # 1 NAs
...
sel<-which(df$Total_amount<=0.0);length(sel) #9 errors
df[sel,"Total_amount"]<-NA 
outlier<-which(df$Total_amount>(iqrvar*3)+quantil3);length(outlier) #X extreme outliers
