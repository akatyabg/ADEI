...
summary(df$f.passanger) #3 missing values
sel<-which(df$Passenger_count<=0.0);length(sel) #2 errors
outlier<-which(df$Passenger_count>6.0);length(outlier) #1 outlier
df[sel,"Passenger_count"]<-NA
df[outlier,"Passenger_count"]<-NA
...
summary(df$f.distance) #60 missing values
sel<-which(df$Trip_distance<=0.0);length(sel) #60 errors

ll<-which(is.na(df$Trip_distance));ll
if(length(ll)>0){
  dfaux<-df[-ll,]
}
iqrvar<-IQR(df$Trip_distance)
quantil3<-quantile(dfaux$Trip_distance, .75);quantil3 #get 3rd quartile
outlier<-which(df$Trip_distance>(iqrvar*3)+quantil3);length(outlier) #101 extreme outliers

df[outlier,"Trip_distance"]<-NA 
...
# Now one by one describe vars
names(df)
vars_con<-names(df)[c(6,7,8,9,11,12,13,14,15,16,19)] 
vars_dis<-names(df)[c(1,4,5,10,20,21)]
