...
table(res.hcpc$data.clust$clust,kc$cluster)
df$claKM 
 
#assignar 
df[names(kc$cluster),"claHP"]<-factor(res.hcpc$data.clust$clust,labels=paste("kHp-",1:8))
df[names(kc$cluster),"claKM"]<-factor(kc$cluster, levels=c(8,7,2,1,6,5,4,3), labels=c("kKM-8","kKM-7","kKM-2","kKM-1","kKM-6","kKM-5","kKM-4","kKM-3"))
