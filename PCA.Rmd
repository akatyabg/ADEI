...
#According to the elbow rule we have to take the first 6 dimentions as the slope of the graphic shows. We are going to follow this rule, even though makes no difference. 78,6 % of the variance is saved
elbow <- kaiser
...
res.pca<-PCA(df[,c(vars_con_pca, "f.fare_amount", "f.total", "pick_up_period", "f.passenger", "f.pickup_longitude", "f.pickup_latitude", "f.dropoff_longitude", "f.dropoff_latitude", "f.Improvement_surcharge", "f.MTA_tax")], ncp=6, quanti.sup=which(vars_con_pca=="Total_amount"), ind.sup = vec_out, quali.sup = 17:26, graph= FALSE)
...
df$claKM<-factor(df$claKM,levels=c(6,4,1,5,2,3,7),labels=c("kKM-6","kKM-4","kKM-1","kKM-5","kKM-2","kKM-3","kKM-7"))
