...
kaiser <- res.pca$eig[1:length,1] #keep only EV >=1
length(res.pca$eig[,1])
kaiser # first 6 
elbow <- res.pca$eig[1:5,1] #above the elbow
...
```{r}
kMeansCluster <- kmeans(kaiser, 3, nstart = 20)
kMeansCluster$clusterF <-as.factor(kMeansCluster$cluster)

ggplot(kaiser, aes(kaiser[, 1:2], color = kMeansCluster$clusterF)) + geom_point()
```
