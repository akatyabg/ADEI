...
vars_con_out<-vars_con[c(1:4,14:15)]
...
res.pca<-PCA(df[,c(vars_con_pca, "f.fare", "f.tt", "f.hour")], ncp=4, quanti.sup=which(vars_con_pca=="Total_amount"), ind.sup = vec_out, quali.sup = 19:21, graph= FALSE) ##still not working
...
res.hcpc <-HCPC(res.pca, nb.clust = 8)
names(res.hcpc)
...
dim(ppcc)
kc<-kmeans(ppcc,8,iter.max = 30, trace=T)
...
df$claHP<-res.hcpc$data.clust$clust
df$claKM<-kc$cluster
table(df$claHP, df$claKM)

 
#assignar 
df$claHp<-factor(df$claHP,labels=paste("kHp-",1:8))
df$claKM<-factor(df$claKM, levels=c(6,5,7,1,2,4,3,8), labels = "blob") # DO later when everything before it is ready
